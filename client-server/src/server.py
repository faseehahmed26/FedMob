...
class ClientServer:
    def __init__(self, host: str = "0.0.0.0", port: int = 8082, flower_server_address: str = "localhost:8080"):
        ...
        self.message_handler = MessageHandler(
            client_manager=self.client_manager)  # Pass ClientManager
    ...
    async def handle_client(self, websocket: websockets.WebSocketServerProtocol, path: str):
        ...
        try:
            ...
            if data["type"] == "register":
                ...
                print(
                    f"[SERVER] Client {client_id} is now in 'ready' state - waiting for training start")
                ...
                print(
                    f"[SERVER] Sent registration acknowledgment to {client_id}")
            ...
            if client_id:
                ...
                if flower_client:
                    print(
                        f"[SERVER] Cleaning up Flower client for {client_id}")
                    ...
                    print(
                        f"[SERVER] No Flower client to clean up for {client_id} (was in ready state only)")
    ...
    async def handle_message(self, client_id: str, message: Dict):
        ...
        try:
            print(
                f"[SERVER] Handling message from {client_id}: {message.get('type')}")
            ...
            def send_fn(msg): return self.send_message(client_id, msg)
            ...
            if message_type == "start_training":
                ...
                print(
                    f"[SERVER] User requested training start for {client_id}")
                await self.handle_training_start(client_id)
            elif message_type == "training_update":
                progress = message.get("progress", 0)
                print(
                    f"[SERVER] Training progress update for {client_id}: {progress}%")
                self.client_manager.update_training_progress(
                    client_id, progress)
            ...
            print(
                f"[SERVER] Error handling message from client {client_id}: {e}")
            logger.error(
    ...
    async def handle_training_start(self, client_id: str):
        ...
        try:
            ...
            if flower_client:
                print(
                    f"[SERVER] Flower client already exists for {client_id} - training may already be active")
                return
            ...
            if success:
                print(
                    f"[SERVER] Flower client connected to Flower server for {client_id}")
                print(
                    f"[SERVER] FL session active - Flower server will coordinate training rounds")
                ...
                print(
                    f"[SERVER] Failed to connect Flower client to Flower server for {client_id}")
                await self.send_message(client_id, {
    ...
    async def send_message(self, client_id: str, message: Dict):
        ...
        try:
            print(
                f"[SERVER] Sending message to {client_id}: {message.get('type')}")
            client = self.client_manager.get_client(client_id)
            if client:
                # Accept either dict or pre-encoded JSON string
                payload = json.dumps(message) if isinstance(
                    message, dict) else message
                await client.websocket.send(payload)
                print(f"[SERVER] Successfully sent message to {client_id}")
            else:
                print(
                    f"[SERVER] Cannot send message - unknown client {client_id}")
                logger.error(
                    f"Cannot send message - unknown client {client_id}")
        except Exception as e:
    ...
    async def cleanup_inactive_clients(self):
        ...
        while True:
            try:
                inactive_clients = self.client_manager.get_inactive_clients(
    ...
    async def start(self):
        ...
        async with websockets.serve(self.handle_client, self.host, self.port):
            print(
                f"[SERVER] WebSocket server running on {self.host}:{self.port}")
